...
function EvidencePanel({ event }) {
  ...
  return (
    <div style={{padding:"0 0 0 20px",overflowY:"auto",height:"100%"}}>
      ...
      {(statusKey==="MATCH"||statusKey==="matched"||statusKey==="axle_error") && (
        <div style={evStyles.section}>
          ...
          {[
            ...
            ["id_classe",      satClaseLabel(event.id_classe)],
            ["tab_id_classe",  satClaseLabel(event.tab_id_classe)],
        ...
        <div style={mStyles.body}>
          ...
          <div style={mStyles.col}>
            ...
            <div style={mStyles.card}>
              ...
              <Field label="id_classe"      value={event.id_classe != null && event.id_classe !== "" ? satClaseLabel(event.id_classe) : null} />
              <Field label="tab_id_classe"  value={event.tab_id_classe != null && event.tab_id_classe !== "" ? satClaseLabel(event.tab_id_classe) : null} />
...
function LaneDetail({ laneId, onBack }) {
  ...
  const COLS = [
    {key:"id",label:"#",w:50},
    {key:"tipo",label:"Estado",w:130},
    {key:"avc_date",label:"Hora AVC",w:100},
    {key:"sat_date",label:"Hora SAT",w:100},
    {key:"delta_segundos",label:"Δ(s)",w:55},
    {key:"vehicle_type",label:"Tipo",w:100},
    {key:"axle_count",label:"Ejes AVC",w:70},
    {key:"id_classe",label:"Clase SAT",w:80},
    {key:"sat_prix",label:"Monto",w:80},
    {key:"observacion_auditoria",label:"Notas",w:null},
  ];
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",height:"100%"}}>
      <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:16,flexWrap:"wrap"}}>
        ...
        <button style={ldStyles.exportBtn}>CSV</button>
        <button style={ldStyles.exportBtn}>Excel</button>
      </div>
      ...
      <div style={{display:"flex",flex:1,overflow:"hidden"}}>
        <div style={{flex:"0 0 60%",overflowY:"auto",overflowX:"auto"}}>
          {filtered.length > 0 && (
            <div style={{...ldStyles.pagBar,borderTop:"none",borderBottom:"1px solid #1e2535",position:"sticky",top:0,zIndex:3}}>
          ...
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
            <thead>
              <tr style={{background:"#070c18",position:"sticky",top:0,zIndex:2}}>
                {COLS.map(c=>(
                  <th key={c.key} onClick={()=>toggleSort(c.key)} style={{...ldStyles.th,width:c.w||undefined,cursor:"pointer"}}>
                    {c.label}{sortCol===c.key&&<span style={{marginLeft:4,color:"#4d7fe0"}}>{sortDir>0?"↑":"↓"}</span>}
                  </th>
                ))}
                <th style={{...ldStyles.th,width:64,textAlign:"center"}}>Acciones</th>
              </tr>
            </thead>
            <tbody>
              {paginated.map((ev,i)=>{
                ...
                return (
                  <tr key={ev.id||ev.avc_id||i}
                    ...
                    style={{background:isSel?`${meta.color}18`:i%2===0?"#0d1525":"#0b1020",borderLeft:isSel?`3px solid ${meta.color}`:"3px solid transparent",cursor:"pointer",transition:"background 0.1s"}}>
                    <td style={ldStyles.td}>{ev.id||ev.avc_id||i+1}</td>
                    <td style={ldStyles.td}><span style={{color:meta.color,fontSize:11,fontWeight:600}}>{meta.icon} {meta.label}</span></td>
                    <td style={{...ldStyles.td,fontFamily:"monospace",fontSize:11}}>{(ev.avc_date||ev.event_mexico||ev.avcTime||"—").slice(11,19)||ev.avc_date||"—"}</td>
                    ...
                    <td style={ldStyles.td}>{ev.id_classe!=null&&ev.id_classe!==""?(SAT_CODE[ev.id_classe]||ev.id_classe):ev.satClass||"—"}</td>
                    <td style={{...ldStyles.td,color:"#22c97b"}}>{ev.sat_prix||ev.amount||"—"}</td>
                    <td style={{...ldStyles.td,color:"#5b6a8a",fontSize:11,maxWidth:200,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{ev.observacion_auditoria||ev.notes||""}</td>
                    <td style={{...ldStyles.td,textAlign:"center",whiteSpace:"nowrap"}}>
